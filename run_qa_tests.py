"""
Run insurance-deploy tests on Databricks serverless for QA audit fix verification.
"""
...
BASE = "/home/ralph/repos/insurance-deploy"
WORKSPACE_PATH = "/Workspace/burning-cost/insurance-deploy-qa"
...
# MAGIC %pip install numpy scipy pandas joblib pytest pytest-cov --quiet
...
r = subprocess.run(
    [sys.executable, "-m", "pip", "install", "-e",
     "/Workspace/burning-cost/insurance-deploy-qa", "--quiet"],
    capture_output=True, text=True,
)
if r.returncode != 0:
...
tests_path = "/Workspace/burning-cost/insurance-deploy-qa/tests"
...
run = w.jobs.submit(
    run_name="insurance-deploy-qa-pytest",
    tasks=[
        jobs.SubmitTask(
            task_key="pytest",
